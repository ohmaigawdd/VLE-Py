...
class RachfordRice:
    ...
    def checkState(self):
        if sum(self.calcX()) <= 1.01 and sum(self.calcX()) >= 0.99:
            if sum(self.calcY(self.x)) <= 1.01 and sum(self.calcY(self.x)) >= 0.99:
                return "VLE"
            else:
                return "Liquid"
        else:
...
test = RachfordRice(2, -100, 500, ['Methane','Ethylene'], [0.3,0.7])
print(test.get_dets())
print(test.x)
print(test.y)
print(test.v)
